...
def reserves_predicted_scaled(x,n,i,a,m,stress_MT=0,stress_interest_rates=0, adapt=True):
    if adapt==True:
        ## First, We compute the best model
        stress=stress_MT
        stress_i=stress_interest_rates
        newTH=stresstest.StressTest_table(TH,stress)[0]
        annual_premium=  term_insurance_predicted_polynomiale_scaled(x,m,n,i,a,stress,stress_i,degree=8)        
        i=i+stress_i
        listcontract=np.zeros((1,n+1))
        list_annual_premium1=np.zeros((1,n+1))
        list_natural_premium1=np.zeros((1,n+1))        
        ...
        annual_premium=  term_insurance_predicted_polynomiale_scaled(x,m,n,i,a,0,0,degree=8)
        list_annual_premium1=np.zeros((1,n+1))
        list_natural_premium1=np.zeros((1,n+1))        
        stress=stress_MT
        stress_i=stress_interest_rates
    ...
    p1,=plt.plot(np.arange(0,n+1,1),recurrence1,label='Polynomial model reserves')
    p2,=plt.plot(np.arange(0,n+1,1),recurrence2,label='real reserves')
    p3,=plt.plot(np.arange(0,n,1),list_natural_premium2[0][0:n],label='real natural premiums')
    p4,=plt.plot(np.arange(0,n,1),list_annual_premium2[0][0:n],label='real annual premiums')    
    p5,=plt.plot(np.arange(0,n,1),list_natural_premium1[0][0:n],label='Polynomial model natural premiums')
    p6,=plt.plot(np.arange(0,n,1),list_annual_premium1[0][0:n],label='Polynomial annual premiums')  
...
def reserves_sum_scale(stress_MT=0,stress_interest_rates=0,adapt=True): 
    level_annual_premium=np.zeros((len(X),41))
    natural_premium=np.zeros((len(X),41))    
    if adapt==True:
        stress=stress_MT
        stress_i=stress_interest_rates
        newTH=stresstest.StressTest_table(TH,stress)[0]
        data=Polynomial_scaled(8,stress,stress_i)
        listcontract=np.zeros((len(X),41))
        for contract in range(0,len(X)):
           x=int(X.iloc[contract].age)
           m=int(X.iloc[contract].nb_payements)
    ...
    p1,=plt.plot(np.arange(0,41,1),level_annual_premium_total,label='level annual premiums')    
    p3,=plt.plot(np.arange(0,41,1),natural_premium_total,label='natural premium')
    p2,=plt.plot(np.arange(0,41,1),recurrence2,label='Polynomial reserves')
    plt . xlabel ('Years', fontsize =20)
    plt . ylabel ('Reserves', fontsize =20)
    plt . title ('Predicted reserves with stress on mortality table={}'.format(stress*100)+"%"+" and stress on interest rates={}".format(stress_i*100)+"%",fontsize =16)
    plt . legend ( handles =[p1 , p2,p3],fontsize =16)
    #plt.show()
    reserve_total=recurrence2
...
def Polynomial_scaled(degree=8,stress_MT=0,stress_interest_rates=0):
    y_stressed=list()    
    TH_stressed=stresstest.StressTest_table(TH,stress_MT)[0]
    X_new=X.copy()
    X_new.interest_rate=X.interest_rate+stress_interest_rates
    for contract in range(0,len(X)):
...
def term_insurance_predicted_polynomiale_scaled(x,m,n,i,a,stress,stress_i,degree=8):
    ...
    for contract in range(0,len(X)):
        y_stressed.append(stresstest.TermInsuranceAnnual(int(X.iloc[contract].age),int(X.iloc[contract].maturity),X.iloc[contract].interest_rate,X.iloc[contract].amount,int(X.iloc[contract].nb_payements),TH_stressed))    
